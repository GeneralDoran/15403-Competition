...
*/
    ...
    //Utility motors
    DcMotor armMotor, jointMotor, intakeMotor, utilmotor4;
    //odometry encoders
    ...
    public void assignUtilMotors(DcMotor arm, DcMotor joint, DcMotor intake, DcMotor util4) {
        armMotor = arm;
        jointMotor = joint;
        intakeMotor = intake;
        utilmotor4 = util4;
    ...
    */
    ...
    public void encoderLift(double speed, double liftInches,  double timeoutS){
        int heightTarget;
        //int averageTarget;
        double liftSpeed;
        if (opModeIsActive()) {
            ...
            resetSlideEncoders();
            // Determine new target position, and pass to motor controller
            heightTarget = (armMotor.getCurrentPosition() + (int) (liftInches * ENCODER_COUNTS_PER_INCH));
            //averageTarget = ((Math.abs(leftBackTarget) + Math.abs(leftFrontTarget)
            //      +Math.abs(rightFrontTarget) + Math.abs(rightBackTarget))/4);
            armMotor.setTargetPosition(heightTarget);
            //jointMotor.setTargetPosition(heightTarget * -1);
            ...
            //jointMotor.setPower((liftSpeed * -1));
            ...
            while (opModeIsActive() &&
                    (runtime.seconds() < timeoutS)
                    && (armMotor.isBusy()) && (jointMotor.isBusy())) {
                telemetry.addData("Lift Motor Current Position", armMotor.getCurrentPosition());
                telemetry.addData("Lift Motor Desired Position", heightTarget);
                //telemetry.addData("Lift Motor 2 Current Position", jointMotor.getCurrentPosition());
                ...
                telemetry.update();
                liftSpeed = (accelerate(armMotor, liftSpeed, heightTarget));
                armMotor.setPower((liftSpeed));
                jointMotor.setPower((liftSpeed * -1));
            }
    ...
    public void encoderJoint(double speed, double turnInches,  double timeoutS){
        int heightTarget;
        //int averageTarget;
        double liftSpeed;
        if (opModeIsActive()) {
            ...
            resetSlideEncoders();
            // Determine new target position, and pass to motor controller
            heightTarget = (armMotor.getCurrentPosition() + (int) (turnInches * ENCODER_COUNTS_PER_INCH));
            //averageTarget = ((Math.abs(leftBackTarget) + Math.abs(leftFrontTarget)
            //      +Math.abs(rightFrontTarget) + Math.abs(rightBackTarget))/4);
            jointMotor.setTargetPosition(heightTarget);
            ...
            while (opModeIsActive() &&
                    (runtime.seconds() < timeoutS)
                    && (armMotor.isBusy()) && (jointMotor.isBusy())) {
                telemetry.addData("Turn Motor Current Position", armMotor.getCurrentPosition());
                telemetry.addData("Turn Motor Desired Position", heightTarget);
                //telemetry.addData("Lift Motor 2 Current Position", jointMotor.getCurrentPosition());
                //telemetry.addData("Lift Motor 2 Desired Position", heightTarget);
                //telemetry.addData("Average Target",averageTarget);
                telemetry.addData("Turn Speed", liftSpeed);
                //telemetry.addData("Lift 2 Speed", liftSpeed);
                telemetry.update();
                liftSpeed = (accelerate(armMotor, liftSpeed, heightTarget));
                armMotor.setPower((liftSpeed));
